...
breach_report %>% filter(Individuals.Affected<7800) %>% group_by(Primary.Breach) %>% summarise( totals=sum(Individuals.Affected)) %>%  ggplot(aes(x=totals,fill=Primary.Breach))+geom_bar()
...
#look at some models
```r{
brmodel1<- lm(Individuals.Affected~Type.of.Breach,data=breach_report)
summary(brmodel1)
